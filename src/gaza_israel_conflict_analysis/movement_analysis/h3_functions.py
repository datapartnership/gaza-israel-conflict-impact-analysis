...
def geometry_to_h3_indexes(geometry, resolution=9):
    ...
    h3_indexes = h3.polyfill(geojson=geojson_polygon, res=resolution, geo_json_conformant=True)
    return list(h3_indexes)

def h3_to_polygon(h3_index):
    """Convert an H3 index to a Shapely Polygon representing its hexagonal grid."""
    boundary = h3.h3_to_geo_boundary(h3_index, geo_json=True)  # Get the hexagon boundary coordinates
...
def aggregate_movement_to_temporal_aggregation(oct_23, agg_type, temp_agg_type='W-MON'):
    ...
    oct_23_agg = oct_23.groupby([agg_type, 'agg_day_period']).sum('activity_index_total')[['activity_index_total']].reset_index().sort_values(by='agg_day_period')
    oct_23_agg['agg_day_period'] = oct_23_agg['agg_day_period'].apply(lambda x: pd.to_datetime(x))
    oct_23_agg = oct_23_agg.groupby([agg_type, pd.Grouper(key='agg_day_period', freq=temp_agg_type)]).sum().reset_index().sort_values(by=[agg_type,'agg_day_period'])
